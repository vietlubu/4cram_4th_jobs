...
uint64 pc_calc_skilltree_normalize_job(struct map_session_data *sd)
{
	...
	{
		std::shared_ptr<s_job_info> job = job_db.find(pc_mapid2jobid(sd->class_&MAPID_BASEMASK, sd->status.sex));
	...
	if (sd->class_&JOBL_THIRD && (sd->class_&MAPID_THIRDMASK) != MAPID_SUPER_NOVICE_E && !sd->change_level_3rd)
	{
		...
		sd->change_level_3rd = job->max_job_level;
...
int pc_need_trait_point(struct map_session_data* sd, int type, int val)
{
	...
	low = pc_getstat(sd, type);
	max = pc_maxparameter(sd, (enum e_params)(type - SP_POW + PARAM_POW));
...
int pc_maxtraitparameterincrease(struct map_session_data* sd, int type)
{
	...
	max_param = pc_maxparameter(sd, (enum e_params)(type - SP_POW + PARAM_POW));
...
bool pc_traitstatusup(struct map_session_data* sd, int type, int increase)
{
	...
	increase = cap_value(increase, 0, max_increase); // cap to the maximum status points available
	if (increase <= 0 || current + increase > pc_maxparameter(sd, (enum e_params)(type - SP_POW + PARAM_POW))) {
		clif_statusupack(sd, type, 0, 0);
		return false;
	}
...
int pc_traitstatusup2(struct map_session_data* sd, int type, int val)
{
	...
	max = pc_maxparameter(sd, (enum e_params)(type - SP_POW + PARAM_POW)); // set new value
