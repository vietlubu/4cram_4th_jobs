...
static bool itemdb_read_sqldb_sub(std::vector<std::string> str) {
	...
	if (!str[++index].empty())
		classes["Forth"] = std::stoi(str[index]) ? "true" : "false";
	if (!str[++index].empty())
...
#ifdef RENEWAL
			",`magic_attack`,`class_third`,`class_third_upper`,`class_third_baby`,`class_forth`,`job_kagerouoboro`,`job_rebellion`,`job_summoner`"
#endif
