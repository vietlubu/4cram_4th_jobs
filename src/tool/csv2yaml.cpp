...
static bool pc_readdb_job_basehpsp(char* fields[], int columns, int current) {
	...
	if (job_id >= JOB_DRAGON_KNIGHT && job_id <= JOB_IMPERIAL_GUARD2)
		j = 199;// Trait era jobs start off at level 200.
	else if ((job_id >= JOB_RUNE_KNIGHT && job_id <= JOB_BABY_MECHANIC2) || job_id == JOB_KAGEROU || job_id == JOB_OBORO || job_id == JOB_REBELLION || job_id == JOB_BABY_KAGEROU || job_id == JOB_BABY_OBORO || job_id == JOB_BABY_REBELLION || job_id >= JOB_STAR_EMPEROR)
